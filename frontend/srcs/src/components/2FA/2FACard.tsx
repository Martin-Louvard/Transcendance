...
const TwoFACard: React.FC = () => {
    ...
    const generateQr = async () =>{
        ...
        try{
          const response = await fetch(`http://10.33.3.5:3001/2fa/${user?.username}/generate`, requestOptions)
          if (response.ok) {
      ...
      const handleSubmit = async (event: React.FormEvent<HTMLFormElement>)=>{
        ...
        try{
          const response = await fetch(`http://10.33.3.5:3001/2fa/${user?.username}/turn-on`, requestOptions)
          if (response.ok)
          {
