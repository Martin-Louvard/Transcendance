 ...
 navigate("/friends/friendprofile");
  ...
  useEffect(() => {
    async function fetchGames() {
      ...
      try{
          const response = await fetch(`http://${process.env.IP}/game/leaderboard`, requestOptions)
          ...
          {board && board[0] && board?.map((user, index) => (
            ...
            <li className="item" key={index}  onClick={() => {goToUserProfile(user);}}>
                <div className='friend-picture'>
                    <img src={"http://${process.env.IP}/users/avatar/" + user.username + "/" + user.avatar.split("/").reverse()[0]}/>
                  </div>
                  <p>{user.username}</p>
