...
export const MapParams: React.FC = (props) => {
	...
	return (
	  <Stack  sx={{marginX: size.width / 10 / 4, backgroundColor: 'transparent', color:'white'}}>
		  <p>Map parameters: </p>
		  <Stack spacing={1} direction={size.width > 700 ? "row" : "column"} sx={{ mb: 1} } alignItems="center">
			{/*Map Size*/}
			<Stack spacing={2} direction="column" sx={{ mt: 6, mb: 1}} alignItems="center">
			  ...
			  <Stack spacing={0} direction="column" sx={{ mb: 1}} alignItems="center">
				...
				<Stack spacing={5} direction="row" sx={{ mt: 0, mb: 1 }} alignItems="center">
				  <p>Width</p>
				  <Slider  disabled={owner == user?.username || !game.lobbyId ? false : true} value={params.map.size[0]}  defaultValue={50} aria-label="Default" valueLabelDisplay="auto" max={500} min={100} sx={sliderStyle}
				  onChange={(_, val) => {setMapParam(prev => ({...prev, size: [val as number, prev.size[1]]}))}}/>
				</Stack>
				<Stack spacing={5} direction="row" sx={{ mt: 0, mb: 1}} alignItems="center">
				  <p>Height</p>
				  <Slider  disabled={owner == user?.username || !game.lobbyId ? false : true} value={params.map.size[1]} defaultValue={50} aria-label="Default" valueLabelDisplay="auto" max={500} min={100} sx={sliderStyle}
			...
			<Stack spacing={2} direction="column" sx={{ mt: 30, mb: 1}} alignItems="center">
				<p>Goal Size</p>
				<Slider  disabled={owner == user?.username || !game.lobbyId ? false : true} value={params.map.goalSize} defaultValue={0} aria-label="Default" valueLabelDisplay="auto" max={mapParam.size[0]} min={Math.floor(mapParam.size[0] / 8)} sx={sliderStyle}
				onChange={(_, val) => {setMapParam(prev => ({...prev, goalSize: val as number}))}}/>
			</Stack>
			{/*Median Offset Size*/}
			<Stack spacing={2} direction="column" sx={{ mt: 30, mb: 1}} alignItems="center">
				<p>Median Offset</p>
				<Slider disabled={owner == user?.username || !game.lobbyId ? false : true} value={params.map.medianOffset} defaultValue={10} aria-label="Default" valueLabelDisplay="auto" max={40} min={0} sx={sliderStyle}
	...
	return (
	  <Stack sx={{backgroundColor: 'transparent', color:'white'}}>
		...
		<Stack spacing={1} direction={size.width > 700 ? "row" : "column"} sx={{color:'white', mb: 1}} alignItems="center">
			...
			<Stack spacing={0} direction="column" sx={{ mt: 0, mb: 1 }} alignItems="center">
			  <p>Rotation Force</p>
			  <Slider value={params.ball.rotationForce} disabled={owner == user?.username || !game.lobbyId ? false : true}  defaultValue={50} aria-label="Default" valueLabelDisplay="auto" max={500} min={100} sx={sliderStyle}
	...
	return (
	  <Stack sx={{backgroundColor: 'transparent', color:'white'}}>
		...
		<Stack spacing={1} direction={size.width > 700 ? "row" : "column"} sx={{color:'white', mb: 1} } alignItems="center">
			<Stack spacing={0} direction="column" sx={{ mt: 0, mb: 1, }} alignItems="center">
			  <p>Speed</p>
			  <Slider value={params.players.speed} disabled={owner == user?.username || !game.lobbyId ? false : true}  defaultValue={50} aria-label="Default" valueLabelDisplay="auto" max={150} min={10} sx={sliderStyle}
			  onChange={(_, val) => {setPlayersParam(prev => ({...prev, speed: val as number}))}}/>
			</Stack>
			<Stack spacing={0} direction="column" sx={{ mt: 0, mb: 1 }} alignItems="center">
			  <p>Rotation Speed</p>
			  <Slider value={params.players.rotationSpeed} disabled={owner == user?.username || !game.lobbyId ? false : true}  defaultValue={50} aria-label="Default" valueLabelDisplay="auto" max={100} min={10} sx={sliderStyle}
			  onChange={(_, val) => {setPlayersParam(prev => ({...prev, rotationSpeed: val as number}))}}/>
			</Stack>
			<Stack spacing={0} direction="column" sx={{ mt: 0, mb: 1 }} alignItems="center">
			  <p>Boost Force</p>
			  <Slider value={params.players.boostForce} disabled={owner == user?.username || !game.lobbyId ? false : true}  defaultValue={50} aria-label="Default" valueLabelDisplay="auto" max={500} min={100} sx={sliderStyle}
